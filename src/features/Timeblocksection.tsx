      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select aria-label="開始（時）" value={startH} onChange={e => setStartH(e.target.value)} style={{ width: 72 }}>
              {HOURS.map(h => <option key={h} value={pad(h)}>{pad(h)}時</option>)}
            </select>
            <span style={{ color: 'var(--muted)' }}>:</span>
            <select aria-label="開始（分）" value={startM} onChange={e => setStartM(e.target.value)} style={{ width: 62 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select aria-label="終了（時）" value={endH} onChange={e => setEndH(e.target.value)} style={{ width: 72 }}>
              {HOURS.map(h => <option key={h} value={pad(h)}>{pad(h)}時</option>)}
            </select>
            <span style={{ color: 'var(--muted)' }}>:</span>
            <select aria-label="終了（分）" value={endM} onChange={e => setEndM(e.target.value)} style={{ width: 62 }}>
              <option value="00">00</option><option value="30">30</option>
            </select>
          </div>

          <select aria-label="ブロックの色" value={color} onChange={e => setColor(e.target.value as BlockColor)} style={{ width: 110 }}>
